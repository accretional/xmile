...
// rss.Rss) or a typed ParseError verdict. As with Parse the RPC status is OK in
// both cases; a non-OK status is reserved for a genuine server fault.
func (s *Server) ParseRss(_ context.Context, req *xmlpb.ParseRssRequest) (*xmlpb.ParseRssResponse, error) {
	rss, err := ParseRSS(s.parser, string(req.GetXml()))
	if err != nil {
		return &xmlpb.ParseRssResponse{Response: &xmlpb.ParseRssResponse_Error{Error: verdictOf(err)}}, nil
	}
	return &xmlpb.ParseRssResponse{Response: &xmlpb.ParseRssResponse_Rss{Rss: rss}}, nil
}
